 ...
 * Usage: node src/seed/populateIaContent.js
 */
...
async function main() {
  ...
  const furn = after.data?.pages?.furniture;
  console.log("✓ furniture title:", furn?.hero?.title);
  console.log("✓ furniture image:", furn?.hero?.image);
  console.log("✓ furniture sections:", furn?.sections?.length || 0);
  console.log("✓ kitchens child:", furn?.children?.[0]?.hero?.subtitle?.slice?.(0, 60) || furn?.children?.[0]?.hero?.subtitle);
  console.log("✓ services sections:", after.data?.pages?.services?.sections?.length || 0);
  console.log("✓ oppolia sections:", after.data?.pages?.aboutBrand?.children?.find?.((c) => c.slug === "oppolia")?.sections?.length || 0);
  console.log("✓ indexable furniture:", furn?.indexable);
  console.log("Open Admin → Varsovia → IA sections → Page content blocks to edit.");
}
